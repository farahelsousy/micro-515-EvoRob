...
class AntFlatEnvironment(MujocoEnv):
    ...
    def _get_rew(self, x_velocity: float, y_velocity: float, action):
        forward_reward = 10.0 * x_velocity

        ctrl_cost = 0.01 * np.sum(np.square(action))
        drift_penalty = 0.06 * (y_velocity ** 2)

        quat = self.data.qpos[3:7].copy()   # [w, x, y, z]
        orientation_penalty = 0.25 * (quat[1] ** 2 + quat[2] ** 2)

        fall_penalty = 8.0 if self._get_termination() else 0.0
