"""
MICRO-515 Final Project — HyperNEAT-CPG Hybrid Training
=======================================================

Controller:
    HyperNEATCPGController

Body:
    4 symmetric body parameters:
        front_upper, front_lower, rear_upper, rear_lower

Body mapping:
    assignment default range:
        length = (g + 1) / 4 + 0.1
        length in [0.1, 0.6]

EA:
    NSGA-II multi-objective over [flat, ice, hill]

Important:
    Before running this file, save the controller as:
        evorob/world/robot/controllers/hyperneat_cpg.py
"""
...
from evorob.world.robot.controllers.cppn import HyperNEATCPGController
...
class FinalWorld(World):
    def __init__(self):
        self.controller = HyperNEATCPGController(
            input_size=27,
            output_size=8,
            cppn_hidden=16,
            cpg_scale=1.0,
...
def run_multi_task_evolution(
    ...
    if results_dir is None:
        results_dir = join(ROOT_DIR, "results", "hyperneat_cpg_final_project")
    ...
    for gen in range(num_generations):
        pop = ea.ask()
        fitnesses = np.empty((len(pop), 3), dtype=float)
    ...
    with open(score_path, "w") as f:
        f.write("=" * 60 + "\n")
        f.write("MICRO-515 Final Project — HyperNEAT-CPG Training Summary\n")
        f.write("=" * 60 + "\n\n")
...
if __name__ == "__main__":
    run_multi_task_evolution(
        ...
        crossover_prob=0.5,
        ckpt_interval=5,
        results_dir=join(ROOT_DIR, "results", "hyperneat_cpg_48p_500s"),
        random_seed=42,
    )
